...
export const Button: React.FC<React.ButtonHTMLAttributes<HTMLButtonElement> & { 
  ...
  const variants = {
    primary: "bg-gradient-to-r from-ocean-800 to-ocean-500 text-white shadow-lg shadow-ocean-500/20 hover:shadow-xl hover:shadow-ocean-500/30 hover:-translate-y-0.5",
...
export const Card: React.FC<{ children: React.ReactNode, className?: string, noPadding?: boolean }> = ({ 
  ...
  <div className={`bg-white rounded-[1.5rem] border border-slate-100 shadow-[0_10px_32px_rgb(15,23,42,0.04)] transition-all duration-500 hover:shadow-[0_12px_36px_rgb(15,23,42,0.07)] ${noPadding ? '' : 'p-8'} ${className}`}>
    {children}
  </div>
);
...
export const Header: React.FC<{ title: string, subtitle?: string, action?: React.ReactNode }> = ({ 
  ...
  <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-slide-up">
    <div className="space-y-1">
      <p className="text-[10px] font-black uppercase tracking-[0.24em] text-ocean-500">Shark AI SeafoodOS</p>
      <h1 className="text-4xl font-black text-slate-900 tracking-tight uppercase leading-tight">{title}</h1>
      {subtitle && <p className="text-slate-500 font-semibold text-base">{subtitle}</p>}
    </div>
