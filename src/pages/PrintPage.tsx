...
export const PrintPage: React.FC = () => {
  ...
  const isDispatch = type === 'dispatches';
  const docTitle = isDispatch ? 'DELIVERY ORDER / SURAT JALAN' : type === 'receivings' ? 'RECEIVING NOTE' : type === 'expenses' ? 'EXPENSE VOUCHER' : type === 'sales' ? 'SALES INVOICE' : type?.toUpperCase();
  
  const partyValue = (type === 'receivings' || (type === 'expenses' && data.supplierId))
    ? suppliers.find((s: any) => s.id === data.supplierId)?.name || 'General Supplier' 
    : (type === 'sales' || type === 'dispatches' || (type === 'expenses' && data.buyerId))
      ...
      <div className="max-w-[210mm] mx-auto py-6 px-4 print:hidden">
        <div className="bg-white p-4 rounded-2xl shadow-xl border border-slate-200 flex justify-between items-center">
          ...
          <button onClick={() => window.print()} className="flex items-center gap-2 bg-slate-900 text-white px-8 py-3 rounded-xl font-black hover:bg-black transition-all">
            <Printer size={20} /> {isDispatch ? 'PRINT DELIVERY ORDER' : 'PRINT INVOICE'}
          </button>
        </div>
      </div>
      ...
      <div className="max-w-[210mm] mx-auto bg-white shadow-2xl print:shadow-none print:max-w-none print:w-full min-h-[297mm] print:min-h-0 p-[10mm] flex flex-col box-border border border-slate-200 print:border-none print:p-0">
        ...
        <div className="flex justify-between items-start mb-6">
          ...
          <div className="text-right space-y-4">
            <h2 className="text-2xl font-black tracking-tighter text-slate-300 uppercase leading-none">{docTitle}</h2>
            <div className="space-y-0.5 text-[11px]">
              ...
              {isDispatch && <p><span className="font-bold text-slate-400">INVOICE REF:</span> <span className="font-black text-slate-900">#{data.salesId?.toUpperCase()}</span></p>}
        ...
        <div className="grid grid-cols-2 gap-8 mb-6">
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
            <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">{isDispatch ? 'SHIP TO / DELIVERY FOR' : type === 'sales' ? 'BILL TO' : 'FROM SUPPLIER'}</h3>
          ...
          <div className="flex flex-col justify-end text-right pr-4 pb-4">
             <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-0.5">{isDispatch ? 'DELIVERY TERMS' : 'PAYMENT TERMS'}</p>
             <p className="text-xs font-black text-slate-900 uppercase">{isDispatch ? 'FRAGILE / FROZEN' : 'DUE ON RECEIPT'}</p>
          </div>
        </div>
          ...
          <table className="w-full invoice-table">
            <thead>
              <tr className="text-slate-900">
                ...
                {!isDispatch && <th className="py-2 px-2 text-right text-[10px] font-black uppercase tracking-widest w-28">PRICE</th>}
                {!isDispatch && <th className="py-2 px-2 text-right text-[10px] font-black uppercase tracking-widest w-32">TOTAL</th>}
                {isDispatch && <th className="py-2 px-2 text-right text-[10px] font-black uppercase tracking-widest w-40">REMARKS / NOTES</th>}
              </tr>
            </thead>
            <tbody>
              {(data.lines || []).map((line: any, i: number) => {
                const it = items.find((i: any) => i.id === line.itemId);
                const desc = (type === 'receivings' || type === 'sales' || type === 'dispatches')
                  ? getItemLabel(it)
                ...
                return (
                  <tr key={i}>
                    ...
                    {isDispatch && (
                      <td className="py-2 px-2 text-right font-bold text-slate-400 text-[10px] italic">
                        Good Condition
                      </td>
                    )}
                  </tr>
          ...
          <div className="col-span-7 space-y-4">
            ...
            <div className="space-y-1">
              ...
              <ul className="text-[8px] font-bold text-slate-400 space-y-0.5 list-disc pl-3">
                <li>Goods received are in accordance with the specified quality and quantity.</li>
                {isDispatch ? (
                    <li>This document serves as proof of physical delivery from our warehouse.</li>
                ) : (
            ...
            <div className="space-y-2 p-4 bg-slate-50 rounded-2xl border border-slate-100">
              {isDispatch ? (
                  <div className="flex justify-between items-center text-[11px]">
                    <span className="font-bold text-slate-400 uppercase tracking-widest">TOTAL ITEMS</span>
                    <span className="font-black text-slate-900">{(data.lines || []).length} ITEMS</span>
                  </div>
              ) : (
                  <div className="flex justify-between items-center text-[11px]">
              ...
              <div className="flex justify-between items-center text-[11px]">
                <span className="font-bold text-slate-400 uppercase tracking-widest">TOTAL WEIGHT (KG)</span>
              ...
              {isDispatch && (
                  <div className="pt-3 border-t-2 border-slate-900 flex justify-between items-center mt-2">
                    <span className="text-[11px] font-black text-slate-900 uppercase tracking-[0.2em]">DELIVERY STATUS</span>
                    <span className="text-lg font-black text-slate-900 tracking-tighter uppercase">DISPATCHED</span>
                  </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-10 grid grid-cols-3 gap-6">
          ...
          <div className="text-center space-y-10">
            ...
            <div className="border-b border-slate-900 mx-8"></div>
            <p className="text-[9px] font-black text-slate-900 uppercase">{isDispatch ? 'Driver / Recipient' : 'Customer / Supplier'}</p>
          </div>
        </div>
